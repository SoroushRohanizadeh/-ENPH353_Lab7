...
class Gazebo_Linefollow_Env(gazebo_env.GazeboEnv):
    ...
    def howManyLinePixels(self, pixels):
        """
    ...
    def process_image(self, data):
        ...
        ret, binarized = cv2.threshold(grayscale, BLACK_THRESHOLD, 255, cv2.THRESH_BINARY)
        ...
        sectionWidth = dilated.shape[1] // 10

        # take the first fully line section
        numLinePixelsPerSection = []
        for i in range(10):
            section = dilated[yLimit, sectionWidth * i : sectionWidth * (i + 1)]
            numLinePixelsPerSection.append(self.howManyLinePixels(section))

        maxNumPixels = max(numLinePixelsPerSection)

        if maxNumPixels > 0:
            state[numLinePixelsPerSection.index(maxNumPixels)] = 1
            self.timeout = 0
        else:
            print("no pixels")
            self.timeout += 1
